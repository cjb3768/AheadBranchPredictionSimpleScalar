...
//DBPB has 1k entries, 4 way associativity
#define DBPB_SETS 256	
#define DBPB_ASSOC 4
//SBPB has 64 entries, 8 way associativity
#define SBPB_SETS 8
#define SBPB_ASSOC 8
//TPRT
...
enum bpred_class {
  ...
  BPred_NUM,
  BPredAhead            /* ahead branch prediction */
};
...
/* (AHEAD) Create struct for TPRT entries */
struct tprt_ent_t {
  ...
  enum md_opcode op;		/* opcode of branch corresp. to addr */
  md_addr_t target;		/* last destination of branch when taken */
  struct tprt_ent_t *prev, *next; /* lru chaining pointers */
};
...
struct tpht_ent_t{
   ...
   struct tpht_ent_t *prev, *next; /* lru chaining pointers */
}
...
struct dbpb_t {
	unsigned int sets;
	unsigned int assoc;
	struct dbpb_ent_t *dbpb_data;
};

/* (AHEAD) Create struct for tracking multiple targets and their frequency */
...
struct sbpb_ent_t {
	...
	struct targ_count_pair *target_pairs; /* data entries in table*/
  	struct sbpb_ent_t *prev, *next; /* lru chaining pointers */
}


/* (AHEAD) Create struct for Speculative Branch Predictor Buffer*/
struct sbpb_t {
	unsigned int sets;
	unsigned int assoc;
	struct sbpb_ent_t *sbpb_data;
};

/* (AHEAD) Create struct for TPHT */
struct tpht_t {
	unsigned int size;
	struct tpht_ent_t *tpht_data;
}

/* (AHEAD) Create struct for TPRT */
struct tprt_t {
	unsigned int size;
	struct tprt_ent_t *tprt_data;
}
...
struct bpred_t {
  ...
  /* (AHEAD) Define prediction table, ebtbs, last target register, last branch register, and other things */

  struct btb_t pred_table; //"small table" from paper
  struct dbpb_t dbpb; //ebtb0 from paper
  struct sbpt_t sbpb; //ebtb1 from paper
  struct tprt_t tprt; //target path register table
  struct tpht_t tpht; //target path history table
  unsigned int gbhsr; //global branch history shift register
