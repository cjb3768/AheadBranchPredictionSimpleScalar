	     ...
	     unsigned int retstack_size) /* num entries in ret-addr stack */
{
  ...
  case BPredAhead:
	...
	//sbpb
	if (!(pred->sbpb.sbpb_data = calloc(SBPB_SETS, sizeof(struct sbpb_ent_t))))
	...
	if (pred->sbpb.assoc > 1)
	{
	for (i=0; i < pred->sbpb.sets; i++)
	  {
	    if (i % pred->sbpb.sets != pred->sbpb.sets - 1)
	      pred->sbpb.sbpb_data[i].next = &pred->sbpb.sbpb_data[i+1];
	    else
	      pred->sbpb.sbpb_data[i].next = NULL;
	    
	    if (i % pred->sbpb.sets != pred->sbpb.sets - 1)
	      pred->sbpb.sbpb_data[i+1].prev = &pred->sbpb.sbpb_data[i];
	  }
        }
	...
	for (i=0; i < pred->tprt.size; i++)
	  {
	    if (i != pred->tprt.size - 1)
	      pred->tprt.tprt_data[i].next = &pred->tprt.tprt_data[i+1];
	    else
	      pred->tprt.tprt_data[i].next = NULL;
	    
	    if (i != pred->tprt.size - 1)
	      pred->tprt.tprt_data[i+1].prev = &pred->tprt.tprt_data[i];
	  }
	...
	for (i=0; i < pred->tpht.size; i++)
	  {
	    if (i != pred->tpht.size - 1)
	      pred->tpht.tpht_data[i].next = &pred->tpht.tpht_data[i+1];
	    else
	      pred->tpht.tpht_data[i].next = NULL;
	    
	    if (i != pred->tpht.size - 1)
	      pred->tpht.tpht_data[i+1].prev = &pred->tpht.tpht_data[i];
	  }
	...
	//gbhsr
	if (!(pred->gbhsr = calloc(1, sizeof(unsigned int))))
		fatal("cannot allocate gbhsr");
	     ...
	     FILE *stream)		/* output stream */
{
  ...
  case BPredAhead:
    ...
    //pred_table
	fprintf(stream, "small table: %d sets x %d associativity", 
	    pred->pred_table.sets, pred->pred_table.assoc);
    //dbpb
	fprintf(stream, "dbpb: %d sets x %d associativity", 
	    pred->dbpb.sets, pred->dbpb.assoc);
    //sbpb
	fprintf(stream, "sbpb: %d sets x %d associativity", 
	    pred->sbpb.sets, pred->sbpb.assoc);
    //tprt
	fprintf(stream, "tprt: %d size", 
	    pred->tprt.size);
    //tpht
	fprintf(stream, "tpht: %d size", 
	    pred->tpht.size);
